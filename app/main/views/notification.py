...
@main.route('/email/notification', methods=['POST'])
def create_email_notification():
    if not current_app.config['EMAIL_ENABLED']:
        return jsonify(error="EMAIL is unavailable"), 503
    notification_request = get_json_from_request('notification')

    validation_result, validation_errors = valid_email_notification(notification_request)
    ...
    if service.restricted:
        if not notification_request['to'] in [user.email_address for user in service.users]:
            abort(400, "Restricted service: cannot send notification")
